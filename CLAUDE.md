...
packages/auth/     Better Auth config only
...
`context.ts` (db plus the Better Auth session, both per request), `trpc.ts` (superjson, `publicProcedure`, `protectedProcedure`), `routers/`, `root.ts` exporting `appRouter` and `AppRouter`.
...
Built and typechecking: `apps/web`, `@repo/ui` (~61 components), `@repo/db`, `@repo/api`, `@repo/jobs`, `@repo/schema`, `@repo/auth`.
...
Not yet built: `packages/ai`, any test setup, and email sync.

Auth is Google OAuth only. Email and password is off in `packages/auth/src/server.ts` until there is a mail transport, and there is no workspace membership yet — signing in gives you a session, not a workspace (RELIV-30).
