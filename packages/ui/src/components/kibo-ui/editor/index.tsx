...
// The suggestion hands `command` the chosen item, not mention attrs — the
// default `SlashNodeAttrs` described the token this used to insert.
const Slash = Node.create<SlashOptions<SuggestionItem, SuggestionItem>>({
  name: "slash",
  priority: 101,
  addOptions() {
    return {
      ...
      suggestion: {
        ...
        // Run the chosen item's own command. Upstream inserted a mention-style
        // `slash` token here instead, so picking "Bullet list" wrote a literal
        // `/bullet-list` node and never toggled the list — the menu appeared to
        // do nothing, and the note carried a node nothing renders.
        ...
        allow: ({ state, range }) => {
          ...
          // noUncheckedIndexedAccess: a schema lookup by name can miss.
          const allow =
...
export const EditorProvider = ({
  ...
  const defaultExtensions = [
    ...
    Slash.configure({
      suggestion: {
        items: async (props) => {
          // Forward the whole props object — tiptap 3.30 also passes `signal`.
          const items = await defaultSlashSuggestions(props);
          const { query } = props;
  ...
  return (
    <Popover modal onOpenChange={onOpenChange} open={open}>
      ...
      >
        ...
        <p
          className={cn(
            "decoration-text-muted text-xs underline underline-offset-4",
            {
      ...
      <PopoverContent align="start" className="w-60 p-0" sideOffset={10}>
        <form className="flex p-1" onSubmit={handleSubmit}>
          ...
          {editor.getAttributes("link").href ? (
            <Button
              className="hover:bg-destructive-foreground flex h-8 items-center rounded-sm p-1 text-destructive transition-all dark:hover:bg-destructive"
              onClick={() => {
...
export const EditorCharacterCount = {
  Characters({ children, className }: EditorCharacterCountProps) {
    ...
    return (
      <div
        className={cn(
          "absolute right-4 bottom-4 rounded-md border bg-background p-2 text-sm text-muted-foreground shadow",
          className,
        )}
      >
        {children}
